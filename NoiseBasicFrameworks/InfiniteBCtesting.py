...
Segments = [100, 300, 500]  # Assume the sensor locates at 250 m from the upstream reservoir and the source locates 750 m  from the upstream reservoir.
Total_Pipe_length = 900
...
Pipe_friction_factor = [0.072, 0.072, 0.072]
SteadyStateVelocity = [0.71,0.71,0.71]
a = 1000
g = 9.81
...
for i in tqdm(range(1, len(freq_range))):
    ...
    for segmentID in reversed(range(len(Segments))):
        ...
        if feature_mapping[segmentID] == "Normal":
            ...
            F = S@F
        Field = FieldMatrix(BasicPipeProperties, length, freq)
        F = Field@F
    c1 = F[0][0]
    c2 = F[0][1]
    c3 = F[0][2]
    ...
    for segmentID in reversed(range(len(Segments))):
        ...
        if feature_mapping[segmentID] == "Normal":
            ...
            F = S@F
        F = FieldMatrix(BasicPipeProperties, length, freq)@F
