...
import TransferMatrix_BackUp.NoiseAnalysis as NoiseAnalysis
import TransferMatrix_BackUp.NormalAnalysis as NormalAnalysis
from TransferMatrix_BackUp.TMTools import *
import multiprocessing as mp
import os
import time
from tkinter import filedialog
...
def plotImpulseResponse(time, Sensor1, Sensor2, Title):
    IRIndex = np.argwhere(abs(Sensor1) > 0.01)
    for i in IRIndex:
        print("IR", time[i] - 40, Sensor1[i])
    print("end-------------------------------")
    plt.figure(Title)
...
def plotCorrelation(time, Sensor1, Sensor2, Title):
    ...
    CCIndexese = np.argwhere(abs(CrossCorrelation) > 40)
    for i in CCIndexese:
        print("CC", time[i] - 40, CrossCorrelation[i])
    print("end-------------------------------")
    plt.figure(Title)
