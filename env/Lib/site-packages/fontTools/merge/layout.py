...
def merge(self, m, tables):
    ...
    for i, (table, dups) in enumerate(zip(tables, m.duplicateGlyphsPerFont)):
        ...
        if table is None or table is NotImplemented:
            log.warning("Have non-identical duplicates to resolve for '%s' but no GSUB. Are duplicates intended?: %s",
                        m.fonts[i]._merger__name, dups)
            continue
...
def layoutPostMerge(font):
    ...
    for t in [GSUB, GPOS]:
        ...
        if t.table.FeatureList and t.table.ScriptList:
            ...
            t.table.FeatureList.FeatureRecord = [f for i, f in enumerate(t.table.FeatureList.FeatureRecord) if
                                                 i in usedIndices]
