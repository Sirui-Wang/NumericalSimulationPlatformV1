...
def _buildOutlineContourFormat1(pen, contour, validate):
    ...
    pen.beginPath()
    if len(contour):
        massaged = _validateAndMassagePointStructures(contour, pointAttributesFormat1, openContourOffCurveLeniency=True,
                                                      validate=validate)
...
def _buildOutlineContourFormat2(pen, contour, identifiers, validate):
    ...
    try:
        ...
        pen.beginPath()
        warn("The beginPath method needs an identifier kwarg. The contour's identifier value has been discarded.",
             DeprecationWarning)
    if len(contour):
...
def _buildOutlinePointsFormat2(pen, contour, identifiers, validate):
    for point in contour:
        ...
        try:
            ...
            pen.addPoint((x, y), segmentType=segmentType, smooth=smooth, name=name)
            warn("The addPoint method needs an identifier kwarg. The point's identifier value has been discarded.",
                 DeprecationWarning)
...
def _buildOutlineComponentFormat2(pen, component, identifiers, validate):
    ...
    try:
        ...
        pen.addComponent(baseGlyphName, tuple(transformation))
        warn("The addComponent method needs an identifier kwarg. The component's identifier value has been discarded.",
             DeprecationWarning)
