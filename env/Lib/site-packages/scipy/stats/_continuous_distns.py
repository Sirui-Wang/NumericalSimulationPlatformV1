...
class levy_stable_gen(rv_continuous):
    ...
    def _pdf_from_cf_with_fft(cf, h=0.01, q=9):
        ...
        N = 2 ** q
        n = np.arange(1, N + 1)
        density = ((-1) ** (n - 1 - N / 2)) * np.fft.fft(
            ((-1) ** (n - 1)) * cf(2 * np.pi * (n - 1 - N / 2) / h / N)) / h / N
        x = (n - 1 - N / 2) * h
        return (x, density)
    ...
    def _pdf_single_value_cf_integrate(x, alpha, beta):
        ...
        return integrate.quad(lambda t: np.real(np.exp(-1j * t * x) * cf(t)), -np.inf, np.inf, limit=1000)[
                   0] / np.pi / 2
        ...
        if alpha != 1:
            ...
            if beta != 0:
                ...
                with np.errstate(all="ignore"):
                    ...
                    intg = integrate.fixed_quad(f, -np.pi / 2, intg_max.x)[0] + \
    ...
    def _pdf(self, x, alpha, beta):
        ...
        for pair in uniq_param_pairs:
            ...
            if fft_min_points_threshold is None or len(data_subset) < fft_min_points_threshold:
                ...
                h = fft_grid_spacing
                q = np.ceil(
                    np.log(2 * np.max(np.abs(_x)) / h) / np.log(2)) + 2 if fft_n_points_two_power is None else int(
    ...
    def _fitstart(self, data):
        ...
        delta = np.clip(zeta - beta * c * np.tan(np.pi * alpha / 2.) if alpha == 1. else zeta, np.finfo(float).eps,
                        np.inf)
